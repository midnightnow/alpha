...
ETA = 0.95  # Vitrification Threshold
PSI = 0.1237 # Hades Gap Constant
PHI = (1 + math.sqrt(5)) / 2
...
class DemiurgeEngine:
    ...
    def draw_demiurge(self, t, is_reflection=False):
        ...
        pygame.draw.line(self.screen, beam_color, staff_top, origin, 4 if tension > ETA else 2)

        # TRIGGER CONDITIONS: The Irrational Remainder breaches the void.
        if not is_reflection and tension > ETA:
            self.trigger_creation((origin[0], origin[1]))
            # Distortion increases as a function of metabolic stress scaled by PSI
            self.distortion_level += (tension - ETA) * 5.0 * (1 / PSI)
