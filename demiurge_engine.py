...
class DemiurgeEngine:
    ...
    def draw_demiurge(self, t, is_reflection=False):
        ...
        # -----------------------------------------------------------------
        # THE 5-12-13 KEY FORMULATED: THE STAFF STRIKE 
        # -----------------------------------------------------------------
        ...
        pygame.draw.line(self.screen, beam_color, staff_top, origin, 4 if random.random() > 0.9 else 2)

        # TRIGGER CONDITIONS: The Irrational Remainder breaches the void.
        if not is_reflection and math.sin(t * 12) > 0.95:
