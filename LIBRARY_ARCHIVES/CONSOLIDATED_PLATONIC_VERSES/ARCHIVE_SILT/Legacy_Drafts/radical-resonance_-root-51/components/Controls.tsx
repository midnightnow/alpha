...
export const Controls: React.FC<Props> = ({
  mode, setMode, params, setParams, isPlayingAudio, toggleAudio, onExport
}) => {
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none flex flex-col justify-between p-6">
      ...
      <div className="flex justify-between items-start pointer-events-auto">
        <div>
          <h1 className="text-2xl font-bold font-mono text-white tracking-tighter">
            ROOT<span className="text-cyan-400">51</span> <span className="text-gray-500 text-lg">BRIDGE</span>
          </h1>
          <p className="text-xs text-gray-400 font-mono mt-1">
            THE PROOF OF REJECTION — √48 vs √51
          </p>
        </div>
      ...
      <div className="pointer-events-auto bg-gray-950/90 backdrop-blur-md border border-gray-800 rounded-lg p-4 w-full max-w-md self-center md:self-start md:mb-0 mb-4 shadow-2xl">
        ...
        {/* Prime Intrusion Toggle — THE PROOF OF REJECTION */}
        <div className="mb-4">
          <div className="flex items-center gap-2 text-xs font-mono text-gray-500 mb-2">
            <RefreshCw size={14} /> GEOMETRIC SIEVE
          </div>
          <button
            onClick={() => {
              ...
              setParams({
                ...
                fracture: newMode === 'fracture'
              });
            }}
            className={`w-full py-3 mb-2 text-xs font-mono border rounded flex items-center justify-center gap-2 transition-all duration-500 ${params.harmonicMode === 'fracture'
              ? 'bg-red-900/40 border-red-500 text-red-400 shadow-[0_0_15px_rgba(255,0,0,0.15)]'
              : 'bg-blue-900/20 border-blue-400/50 text-blue-300/80'}`}
          >
            <Layers size={14} /> {params.harmonicMode === 'fracture' ? '√51 ACTIVE — PRIME 17 INTRUSION' : '√48 SMOOTH — HARMONIC CLOSURE'}
          </button>

          {/* Beat Frequency Display — The Acoustic Proof */}
          <div className={`text-[10px] font-mono text-center py-2 px-3 rounded border transition-all duration-500 ${params.harmonicMode === 'fracture'
            ? 'bg-cyan-950/30 border-cyan-800 text-cyan-400'
            : 'bg-gray-900/50 border-gray-800 text-gray-500'
            }`}>
            <div className="flex justify-between mb-1">
              <span>BEAT FREQ:</span>
              <span className={params.harmonicMode === 'fracture' ? 'text-cyan-300 font-bold' : 'text-gray-600'}>
                {params.harmonicMode === 'fracture' ? '0.660 Hz ✓ HADES ALIGNED' : '0.447 Hz ✗ NO ALIGNMENT'}
              </span>
            </div>
            <div className="flex justify-between mb-1">
              <span>PRIME 17:</span>
              <span className={params.harmonicMode === 'fracture' ? 'text-red-400 font-bold' : 'text-gray-600'}>
                {params.harmonicMode === 'fracture' ? 'PRESENT (3×17)' : 'ABSENT (2⁴×3)'}
              </span>
            </div>
            <div className="flex justify-between">
              <span>KILN STATE:</span>
              <span className={params.harmonicMode === 'fracture' ? 'text-orange-400' : 'text-blue-300/50'}>
                {params.harmonicMode === 'fracture' ? 'VITRIFIED — TEETH VISIBLE' : 'RAW — SMOOTH CLAY'}
              </span>
            </div>
          </div>
        </div>
